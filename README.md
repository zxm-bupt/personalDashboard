...
- 任务标题、描述、DDL、重要程度
- 任务状态：待办 / 进行中 / 已完成
- 勾选完成时任务卡片向下滑出，再落入已完成分组（跟随系统的“减少动态效果”设置）
- 每个任务可关联多个资源：网页 URL / 本地文件 / 本地应用
- Dashboard 中直接点击打开资源
- 每个任务可以“一键打开全部”关联的网页、文件和本地应用
...
### 系统托盘（仅桌面端）
- macOS 使用单色模板图标，跟随菜单栏浅色 / 深色自动反色
- 图标旁不显示文字，点击展开菜单后才显示打卡时间和已工作时长
- 菜单内直接上下班打卡
- 菜单内开始 / 暂停 / 继续番茄钟，并显示当前阶段和剩余分钟
- 关闭主窗口只隐藏界面，计时继续，从托盘“显示主界面”恢复
- 通过托盘菜单“退出”彻底关闭应用

### 时间记录
- 上下班打卡，下班后仍显示当天的上下班时间和累计工作时长
- 任务专注计时
- 当前计时实时显示
- 最近时间明细
- 打卡历史
